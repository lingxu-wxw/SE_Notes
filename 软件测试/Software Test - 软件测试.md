...
- 黑盒测试，等价类，边界值，决策表，这些概念要知道

  - 边界值能够设计多少个测试用例，`xN+m`

- 白盒测试，占考试的30-40%，基于结构路径的测试，基于数据流的测试，大部分东西都考

  - 这是两类，每一类中都有好几个
  - 基于路径有很多覆盖率的指标；基于数据流的也有
  - 基本路径的概念，跟圈复杂度有关系
...
- 集成测试和系统测试
  ...
  - 要会区分这两种测试的区别
  - 集成测试通过什么线索去做集成
  - 集成：自底向上，自顶向下，三明治等集成方法；开发桩程序，用于模拟某些模块的功能
  - MM路径是集成测试的概念，可以用来做集成测试，注意区分DD路径

- 其他

  - 掌握每一种覆盖率的关系，路径的覆盖的几种指标，数据流的也要
  - 重点掌握DD路径和MM路径的分析方式
  - 关注圈数的计算，在很多环节用到了好几次，比如分析程序的复杂度（基本路径的个数，就是圈数的个数）

- 专题性测试（40%）

- 回归测试

  - 掌握一种回归测试的用例选择方法，好像有三种
  - 基于执行轨迹的回归测试方法，知道回归测试的基本流程
  - 为什么会出现回归测试？（是在迭代开发过程中，减少测试工作量

- 变异测试

  - 变异测试为什么会出现？已经测试了的工作，要增强测试的充分性（目的）
  - 变异测试，要知道等价变异体这个概念，如果去找等价变异体
  - 变异体的产生是模拟（熟练）程序员的错误

- 嵌入式测试

  - 循环，路径，cache，等，都会影响测试的时间
  - 软实时和硬实时的区分
  - 交叉测试，和通用测试不一样
...
- 面向对象的测试
  ...
  * 根据UML（用例图）进行测试线索的发现
  * 用例图用在系统测试，顺序图用在集成测试
  * 数据流的测试在面向对象中的概念不太一样（之前过程里就是定义和使用的概念），但在面向对象里面定义和使用不是很一样，会比较复杂
  * 基于事件的paki网
